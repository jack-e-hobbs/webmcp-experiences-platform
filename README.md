...
2.  **Enable the Flag:** Navigate to `chrome://flags/#enable-webmcp-testing` and set it to **Enabled**.
3.  **Use an Agent:** The WebMCP API is browser-provided — no extension needed in Chrome 149+. Use a CLI agent with `chrome-devtools` capabilities, or a browser extension that bridges WebMCP to an LLM.
4.  **Try a Prompt:** Open the [Live Demo](https://jack-e-hobbs.github.io/webmcp-experiences-platform/) and tell your agent: *"Find me a romantic picnic in Melbourne for next weekend."*
...
| **Security** | Implicit (simulated clicks) | Explicit (Human-in-the-Loop via `readOnlyHint`) |
| **State Awareness** | Inferred from text | Exposed via read-only tools |
...
We register business logic tools via `document.modelContext.registerTool`. Each tool (e.g., `initiate_booking`, `search_experiences`) is backed by a strict JSON Schema contract and read/write annotations, ensuring deterministic agent interaction.

### 3. State Awareness (Read-Only Tools)
Page state is exposed to agents via **read-only tools** (marked `readOnlyHint: true`), such as `audit_capabilities` and `get_wishlist`. There is no `provideContext` call — the current spec removes that mechanism.

---

## Analytics & Attribution (Amplitude)

A core focus of this project is measuring and distinguishing between pure human users, and users who interact via LLM sidecars rather than the traditional UI.

*   **Capability Detection:** A session-gated `WebMCP Capability Detected` event fires once per session, with `webmcp_capable` set as a user property.
...
*Canonical API description:* `../skill/webmcp-native-skill/references/webmcp-spec-2026.md`
